 ...
 * The commenting is pretty solid, so the code should be easy
 * 	to understand.
 * 
 * NOTES:
...
void ProbereqSniffer::run(const std::string& iface) {
     ...
     * Since sniffer.next_packet() enter a loop
     *  which it exits only when a new packet is
     *  found, even if the timeout occurs, the thread
     *  will not be stopped until at least a packet
     *  is found.
     * If, when a packet is found, the timeout
     *  has occurred, this packet will be written in
     *  the output file and the thread will be killed.
     *
     * NOTE: If that solution is not good for you, patch
     *  the chrono loop in the libtins library -> sniffer.cpp
	 */
